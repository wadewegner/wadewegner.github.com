...
Remaining in the “Build Settings” tab, now type in “other linker flags” in to the search bar. In the “Other Linker Flags” section, add two Flags. **–ObjC** and **–all\_load**
...
credential = [WAAuthenticationCredential credentialWithAzureServiceAccount:@“ACCOUNT\_NAME” accessKey:@“ACCOUNT\_KEY”];
...
Be sure to replace ACCOUNT\_NAME and ACCOUNT\_KEY with your Windows Azure storage account name and key, available on the Windows Azure portal ([*http://windows.azure.com*](http://windows.azure.com)).
...
credential = [WAAuthenticationCredential authenticateCredentialWithProxyURL:[NSURL URLWithString:@“PROXY\_URL”] user:@“USERNAME” password:@“PASSWORD” withCompletionHandler:^(NSError \*error)
{
if (error)
{
NSLog(@"%@",[error localizedDescription]);
...
Replace the PROXY\_URL, USERNAME, and PASSWORD with the information required to access your proxy service.
